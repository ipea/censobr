...
Some labelled variables have an optional alias and description to make their
meaning easier to identify. Use `label_variable_metadata()` to inspect this
catalogue. It is metadata only: the columns returned by the data-reading
functions keep their original names.

```{r, eval = FALSE}
label_variable_metadata(
  dataset = "population",
  year = 2010
)
```
