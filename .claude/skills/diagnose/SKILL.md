...
- A number changed between runs and you need to localize *which* step drifted.
...
1. Fix every source of nondeterminism: pin the working directory, record `sessionInfo()`, and note the active `censobr_env$data_release` and cache directory.
...
3. Re-run the **full** unit and any dependent step → confirm the fix didn't move another number. If the result feeds a manuscript claim, re-check it (re-run the affected tests).
...
- **Re-run the whole check suite** — that is `/r-package-check`. Diagnose fixes one bug deeply.
