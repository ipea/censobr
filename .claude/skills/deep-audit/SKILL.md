...
4. Rule `paths:` ↔ skill implementation parity (a rule claims a skill follows its protocol, but the skill body has none of the protocol keywords). Also flag any rule whose `paths:` globs match **zero** files in this repo — that rule is dead weight.
...
Launch these 4 agents simultaneously using `Task` with `subagent_type=general-purpose`. Each agent's prompt **must** tell it to check the claim-vs-reality lens explicitly before reporting clean: does every documented command, path, count, and cross-reference actually resolve in this repo?
...
| Always-on rule listing | CLAUDE.md | a rule active here but omitted from CLAUDE.md's listing |
