...
> "Does the entry follow the schema documented in [MEMORY.md](MEMORY.md): `[LEARN:category] wrong → right` for corrections, with the why made explicit? If it's just a free-form note, vote NO — fix the format first, then re-submit."
...
- **Format critic** — context: the candidate entry + [MEMORY.md](MEMORY.md) for the schema reference.
