...
> **Scoped to censobr.** This repo carries an R-package subset of the fleet. The slide/manuscript
> skills that once filled this table (`/slide-excellence`, `/qa-quarto`, `/review-paper`,
> `/create-lecture`, `/data-analysis`) are **not installed here** — the full set remains at `~/.claude/`.

| Skill | Primitives | Notes |
|-------|-----------|-------|
| `/r-package-check` | verify → review (`r-package-reviewer`) → triage | **The release gate for this repo** — `R CMD check --as-cran` |
| `/commit` | verify (Step 2), score (Step 6) | Halts on failure. **Steps 0/0b are skipped in censobr** — `scripts/` does not exist; see [CLAUDE.md](../../CLAUDE.md) |
